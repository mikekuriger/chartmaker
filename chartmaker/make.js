...
function getProcessCount() {
    let pc = 4; // default
    try {
        //linux let r = execSync(`grep -c "^processor" /proc/cpuinfo`, { encoding: 'utf8'});
        let r = execSync(`sysctl -n hw.ncpu`, { encoding: 'utf8'});
        pc = Number(r.replace('\n', ""));
...
function processFulls(msgid = -1) {
    ...
    for (let idx = 0; idx < parray.length; idx++) { //forEach((index) => {
        ...
        if (charttype === "ifr") {
            ...
            chartname = settings.fullchartlist[parray[idx]][2]; // use alias value for IFR
...
function downloadCharts() {
    ...
    if (fs.existsSync(chartzip)) {
        ...
        cmd = `curl -4 -C - ${charturl} -o ${chartzip}`;
        executeCommand(cmd);
    }
}
...
function processImages() {
    ...
    chartareas.forEach((area) => {
        ...
        // --legacy: GDAL 3.13+ defaults gdal2tiles.py to a new "gdal raster tile"
        // backend that silently produces zero tile files with these options
        // (reports success, exit 0, but writes nothing). --legacy restores the
        // original implementation that actually honors --tiledriver/--processes.
        // --xyz: without it, output is TMS-numbered (Y=0 south); the app's
        // Google Maps TileProvider requests Google/OSM XYZ numbering (Y=0 north)
        // with no flip applied anywhere in its code, so omitting --xyz produces
        // tiles at the wrong path entirely (mirrored Y) -- confirmed by exact
        // Y-flip match against known-good tiles.
        cmd = `gdal2tiles.py --legacy --zoom=${settings.zoomrange} --processes=${processes} ${formatargs} --xyz -w none --tilesize 512 -r lanczos ${clipped} ${tiled}`;
        executeCommand(cmd);
    });
}
...
function buildChartNameArray() {
    ...
    let chartnames = [];
    files.forEach((file) => {
        if (file.search("new_york_vfr_plannings.tif") === -1) {
            let fname = file.toLowerCase();
            if ((fname.endsWith(".tif")) &&
                (fname.search("fly") === -1) &&
                // Grand Canyon's "Air Tour Operators" chart covers the exact
                // same extent as "General Aviation" -- mergetiles.pl has no
                // concept of "different product, same area" and will silently
                // composite one over the other. Exclude it so only General
                // Aviation (the one actually wanted) ever gets processed.
                (fname.search("air_tour") === -1)) {
