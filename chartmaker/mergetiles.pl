                    ...
                    if ( -d "$overlay_directory/$zoomlevel/$x" ) {
                        ...
                        foreach my $y (@overlay_y_tiles) {
                            ...
                            if ( -e "$base_directory/$zoomlevel/$x/$y" ) {
                                qx(convert "$base_directory/$zoomlevel/$x/$y" "$overlay_directory/$zoomlevel/$x/$y" -composite "$base_directory/$zoomlevel/$x/$y" 2>/dev/null);
                            }
                            # Otherwise do a regular copy from overlay tile to base directory
                                ...
                                # Recreate an empty marker at the original path so the
                                # source tree stays a complete, permanent record of
                                # which z/x/y tiles belong to this area -- even after
                                # the real file has moved to the merged output. Used
                                # downstream as a tile-membership list (paths only,
                                # never read for content) when building per-area zips.
                                open(my $marker_fh, '>', "$overlay_directory/$zoomlevel/$x/$y");
                                close($marker_fh);
                            }
                        }
                    }
